...
public class EditProfileActivity2 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        databaseReference = FirebaseDatabase.getInstance().getReference().child("Profiles2").child(profileKey);
    ...
    private void retrieveProfileData() {
        databaseReference.get().addOnSuccessListener(snapshot -> {
            if (snapshot.exists()) {
                ...
                Toast.makeText(EditProfileActivity2.this, "Profile not found", Toast.LENGTH_SHORT).show();
                finish();
            }
        }).addOnFailureListener(e -> {
            Toast.makeText(EditProfileActivity2.this, "Failed to retrieve profile", Toast.LENGTH_SHORT).show();
            finish();
        });
    }
    ...
    private void updateProfile() {
        ...
        databaseReference.setValue(profile).addOnSuccessListener(aVoid -> {
            Toast.makeText(EditProfileActivity2.this, "Profile updated successfully", Toast.LENGTH_SHORT).show();
            finish();
        }).addOnFailureListener(e -> {
            Toast.makeText(EditProfileActivity2.this, "Failed to update profile", Toast.LENGTH_SHORT).show();
        });
    }
